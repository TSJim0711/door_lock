    ...
    while(1)
    {
        if(xQueueReceive(msg_queue, &msg_recv, portMAX_DELAY))//wait for event
        {
            ...
            case DOOR_LOCK:
                switch (((doorlock_event_t*)msg_recv.msg_content)->lock_status)
                {
                    case DOOR_UNLOCK:
                        ...
                        break;
                    case DOOR_UNLOCKED:
                        sprintf(print_buff,"%ds left",((doorlock_event_t*)msg_recv.msg_content)->detail/20);
                        strncpy(((view_title_content_t*)(activity_door_unlock->view_structure))->tc_content,print_buff,64);
                        activity_screen_refresh();
                        break;
                    case DOOR_RELOCK:
            ...
            case FG:
                ...
                //fingerprint sensor
                switch (fg_event_content->fg_status) {//translate from common type to self msg_fg_t struct
